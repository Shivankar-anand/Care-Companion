...
public class DocListAdapter extends RecyclerView.Adapter<DocListAdapter.ViewHolder> {
    ...
    public DocListAdapter(Context mcontext, List<ServiceProviders> musers){
        this.musers = musers;
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.info.setText("Call: "+serviceProviders.getPhonenum());

        holder.itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if(mcontext instanceof Main2Activity){
                    Intent intent = new Intent(mcontext,DocChat.class);
                ...
                if(mcontext instanceof Requests){
                    Intent intent = new Intent(mcontext,DocShowSentRequest.class);
                    intent.putExtra("name",serviceProviders.getFirstname()+" "+serviceProviders.getLastname());
                    intent.putExtra("dp",serviceProviders.getProfilePicUrl());
                    intent.putExtra("category",serviceProviders.getCategory());
                    intent.putExtra("phone",serviceProviders.getPhonenum());
                ...
                if(mcontext instanceof selectDoctors) {
                    Intent intent = new Intent(mcontext, profileDoc.class);
                    intent.putExtra("name", serviceProviders.getFirstname() + " " + serviceProviders.getLastname());
                    intent.putExtra("dp", serviceProviders.getProfilePicUrl());
                    intent.putExtra("category", serviceProviders.getCategory());
                    intent.putExtra("phone", serviceProviders.getPhonenum());
                    intent.putExtra("experience", serviceProviders.getYearsOfExperience());
                    intent.putExtra("qualification", serviceProviders.getQualifications());
                    intent.putExtra("userid", serviceProviders.getId());
